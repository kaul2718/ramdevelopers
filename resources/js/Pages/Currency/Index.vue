...
<script setup>
    ...
    import CreateModal from '@/Components/Currency/CreateModal.vue';
    import ViewModal from '@/Components/Currency/ViewModal.vue';
    import EditModal from '@/Components/Currency/EditModal.vue';
    ...
    const openCurrencyModalForEdit = (currency) => {
        selectedCurrency.value = currency;
        editMode.value = true;
    ...
    const openDeleteConfirm = (currency) => {
    ...
    const confirmDelete = () => {
        if (currencyToDelete.value) {
            router.delete(route('currency.destroy', currencyToDelete.value.curr_id), {
                onSuccess: () => {
                    showDeleteConfirm.value = false;
                    currencyToDelete.value = null;
                    
                    notificationStore.success('Moneda eliminada exitosamente');
                ...
                onError: (error) => {
                    ...
                    notificationStore.error('Error al eliminar la moneda');
                    showDeleteConfirm.value = false;
                    currencyToDelete.value = null;
                }
            });
...
<template>
    <AppLayout title="Monedas">
        <HeaderBody titulo-body="Monedas" permisos="create currency" nombre-btn="Crear nueva" @create="showCreateModal = true"/>

        <div class="caja--tabla">
            <table>
                <thead>
                    <tr>
                        <th>Código</th><th>Símbolo</th><th>Nombre</th><th>Posición</th><th>Estado</th><th>Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="item in currencies.data" :key="item.curr_id" @click="openCurrencyModal(item)">
                        <td>{{ item.curr_code }}</td>
                        <td>{{ item.curr_symbol }}</td>
                        <td>{{ item.curr_name }}</td>
                        <td>{{ item.curr_symbol_first ? 'Inicio' : 'Final' }}</td>
                        <td>
                            <div :class="['px-2 rounded-md flex justify-center w-fit items-center gap-2', item.curr_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600']">
                                <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <circle cx="3" cy="3" r="3" :fill="item.curr_active ? '#059669' : '#dc2626'" />
                                </svg>
                                <span>{{ item.curr_active ? 'Activo' : 'Inactivo' }}</span>
                            </div>
                        </td>
                        <td class="botonera--tabla">
                            <button @click.stop="openCurrencyModalForEdit(item)" class="btn--tipo1" title="Editar moneda" v-if="$page.props.user.permissions.includes('update currency')">
                            ...
                            <button @click.stop="openDeleteConfirm(item)"class="btn--tipo2" title="Eliminar moneda" v-if="$page.props.user.permissions.includes('delete currency')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Paginación -->
        <Pagination :objPage="currencies" />

        <!-- Modal de Información de Moneda -->
        <ViewModal 
        ...
        <ConfirmModal
            ...
            message="¿Estás seguro de que deseas eliminar esta moneda? Esta acción es irreversible."
