<!-- filepath: resources/js/Pages/Country/Index.vue -->
<script>
export default {
    name: 'CountryIndex'
}
...
import CountryViewModal from '@/Components/Country/ViewModal.vue';
import CreateModal from '@/Components/Country/CreateModal.vue';
...
// Mostrar notificación después de refrescar
onMounted(() => {
    // Notificación de creación exitosa
    if (sessionStorage.getItem('showCreateCountryNotification')) {
        setTimeout(() => {
            notificationStore.success('País creado exitosamente');
            sessionStorage.removeItem('showCreateCountryNotification');
    ...
    if (sessionStorage.getItem('showUpdateCountryNotification')) {
        setTimeout(() => {
            notificationStore.success('País actualizado exitosamente');
            sessionStorage.removeItem('showUpdateCountryNotification');
    ...
    if (sessionStorage.getItem('showDeleteCountryNotification')) {
        setTimeout(() => {
            notificationStore.success('País eliminado exitosamente');
            sessionStorage.removeItem('showDeleteCountryNotification');
...
const openCountryModalForEdit = (country) => {
    selectedCountry.value = country;
    editMode.value = true;
    showCountryModal.value = true;
};
...
const openDeleteConfirm = (id) => {
    countryToDelete.value = id;
    showDeleteConfirm.value = true;
};

const confirmDelete = () => {
    if (countryToDelete.value) {
        router.delete(`/countries/${countryToDelete.value}`, {
            onSuccess: () => {
            ...
            onError: (error) => {
                ...
                notificationStore.error('Error al eliminar el país');
                showDeleteConfirm.value = false;
                countryToDelete.value = null;
            }
        });
...
<template>
    <AppLayout>
        <template #header>
            <h1 class="font-semibold text-xl text-gray-800 leading-tight">Países</h1>
        </template>
        <div class="py-12">
                            ...
                            <div class="block">
                                <div class="overflow-x-auto w-full  border rounded-lg border-gray-300">
                                    <table class="w-full rounded-xl">
                                        ...
                                        <tbody class="divide-y divide-gray-300 ">
                                            <tr v-for="item in countries.data" :key="item.ctry_id"
                                                ...
                                                <td
                                                    ...
                                                    <div :class="[
                                                        ...
                                                        item.ctry_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'
                                                    ]">
                                                        <svg width="6" height="6" viewBox="0 0 6 6" fill="none"
                                                            xmlns="http://www.w3.org/2000/svg">
                                                            <circle cx="3" cy="3" r="3"
                                                                :fill="item.ctry_active ? '#059669' : '#dc2626'" />
                                                        </svg>

                                                        <span class="font-semibold text-xs">
                                                            {{ item.ctry_active ? 'Activo' : 'Inactivo' }}
                                                        </span>
                                                    </div>
                                                </td>
                                                ...
                                                <td class="flex p-5 items-center gap-0.5">
                                                    ...
                                                    <button
                                                        @click="openCountryModalForEdit(item)"
                                                        v-if="$page.props.user.permissions.includes('update countries')"
                                                    ...
                                                    <button
                                                        @click="openDeleteConfirm(item.ctry_id)"
                                                        v-if="$page.props.user.permissions.includes('delete countries')"
                                ...
                                <div
                                    ...
                                    <div class="flex flex-1 justify-between sm:hidden">
                                        <Link v-if="countries.prev_page_url"
                                            :href="countries.prev_page_url"
                                            class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                                            Anterior
                                        </Link>
                                        <Link v-if="countries.next_page_url"
                                    ...
                                    <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                                        <div>
                                            <p class="text-sm text-gray-700">
                                                Mostrando
                                                <span class="font-medium">{{ countries.from }}</span>
                                                a
                                                <span class="font-medium">{{ countries.to }}</span>
                                                de
                                        ...
                                        <div>
                                            <nav aria-label="Pagination"
                                                ...
                                                <!-- Page Numbers -->
                                                <Link v-for="page in countries.last_page" :key="page"
        ...
        <!-- Modal de Información del País -->
        <CountryViewModal 
            :show="showCountryModal" 
            :country="selectedCountry"
            :isEditingInitial="editMode"
        ...
        <ConfirmModal
            ...
            title="Eliminar País"
            message="¿Estás seguro de que deseas eliminar este país? Esta acción no se puede deshacer."
