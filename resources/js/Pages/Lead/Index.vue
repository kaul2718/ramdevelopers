                    ...
                    <tr v-for="item in leads.data" :key="item.lead_id" @click="openViewModal(item)"
                        ...
                        <td class="botonera--tabla">
                            ...
                            <button @click.stop="openEditModal(item)" class="btn--tipo1" title="Editar campo"
                                v-if="$page.props.user.permissions.includes('update leads')">
                            ...
                            <button @click.stop="openConfirmModal(item)" class="btn--tipo2" title="Eliminar campo"
                                v-if="$page.props.user.permissions.includes('delete leads')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                        ...
                        <td>
                            <div class="space-y-2">
                                <span
                                    class="inline-flex items-center rounded-md bg-yellow-50 px-2 text-s font-medium text-yellow-700">
                        ...
                        <td>
                            <span
                                class="inline-flex items-center rounded-md bg-blue-50 px-2 text-s font-medium text-blue-700">
                                {{ item.source?.leadSou_name || 'N/A' }}
                            </span>
                        </td>

                        <td>{{ (item.user?.name && item.user?.lastname) ? `${item.user.name} ${item.user.lastname}` :
                            'Sin asignar' }}</td>
