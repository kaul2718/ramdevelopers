...
<template>
    <AppLayout title="Leads">
        <HeaderBody titulo-body="Listado de Leads" permisos="create leads" nombre-btn="Crear nuevo" @create="showCreateModal = true"/>

        <div class="caja--tabla">
            <table>
                <thead>
                    <tr>
                        <th>ID</th><th>Cliente</th><th>Email</th><th>País</th><th>Desarrollo</th><th>Fuente</th><th>Estado</th><th>Usuario Responsable</th><th>Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="item in leads.data" :key="item.lead_id">
                        <td>{{ item.lead_id }}</td>
                        <td>{{ item.lead_client_name }}</td>
                        <td>{{ item.lead_client_email || 'N/A' }}</td>
                        ...
                        <td>{{ (item.user?.name && item.user?.lastname) ? `${item.user.name} ${item.user.lastname}` : 'Sin asignar' }}</td>                        
                        <td class="botonera--tabla">
                            <button @click="openViewModal(item)" class="btn--tipo1" title="Ver información" v-if="$page.props.user.permissions.includes('read leads')">
                            ...
                            <button @click="openNotesModal(item)" class="btn--tipo1" title="Editar campo" v-if="$page.props.user.permissions.includes('read lead notes')">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-5">
                                    <path d="M7.5 8.25h9m-9 3H12m-9.75 1.51c0 1.6 1.123 2.994 2.707 3.227 1.129.166 2.27.293 3.423.379.35.026.67.21.865.501L12 21l2.755-4.133a1.14 1.14 0 0 1 .865-.501 48.172 48.172 0 0 0 3.423-.379c1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0 0 12 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018Z" />
                                </svg>
                            </button>
                            <button @click="openEditModal(item)" class="btn--tipo1" title="Editar campo" v-if="$page.props.user.permissions.includes('update leads')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <button @click="openDeleteConfirm(item)"class="btn--tipo2" title="Eliminar campo" v-if="$page.props.user.permissions.includes('delete leads')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
