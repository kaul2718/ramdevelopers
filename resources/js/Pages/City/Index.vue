...
<template>
    <AppLayout title="Ciudades">
        <HeaderBody titulo-body="Gestión de ciudades" permisos="create city" nombre-btn="Crear nueva" @create="showCreateModal = true"/>
        
        <div class="caja--tabla">
            <table>
                <thead>
                    <tr>
                        <th>Id</th><th>Nombre</th><th>Región</th><th>País</th><th>Estado</th><th>Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="item in cities.data" :key="item.city_id">
                        <td>{{ item.city_id }}</td>
                        <td>{{ item.city_name }}</td>
                        <td>{{ item.city_region || 'N/A' }}</td>
                        <td>
                        ...
                        <td>
                            <div :class="['py-1.5 px-2.5 rounded-full flex justify-center w-24 items-center gap-2', item.city_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600']">
                                <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <circle cx="3" cy="3" r="3" :fill="item.city_active ? '#059669' : '#dc2626'" />
                                </svg>
                                <span class="font-semibold text-xs">{{ item.city_active ? 'Activo' : 'Inactivo' }}</span>
                            </div>
                        </td>                        
