...
const confirmDelete = () => {
    if (cityToDelete.value) {
        router.delete(route('cities.destroy', cityToDelete.value.city_id), {
            onSuccess: () => {
            ...
            onError: (error) => {
                ...
                notificationStore.error('Error al eliminar la ciudad');
                showDeleteConfirm.value = false;
                cityToDelete.value = null;
            }
        });
...
<template>
    <AppLayout>
        <template #header>
            <h1 class="font-semibold text-xl text-gray-800 leading-tight">Ciudades</h1>
        </template>

        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="p-6 bg-white border-border-gray-200">
                    <div class="flex justify-end pr-2" v-if="$page.props.user.permissions.includes('create city')">
                        <button 
                                            ...
                                            <tr 
                                                ...
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    <div :class="[
                                                        ...
                                                        item.city_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'
                                                    ]">
                                                        <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <circle cx="3" cy="3" r="3" :fill="item.city_active ? '#059669' : '#dc2626'" />
                                                        </svg>
                                                        <span class="font-semibold text-xs">
                                                            {{ item.city_active ? 'Activo' : 'Inactivo' }}
                                                        </span>
                                                    </div>
                                                </td>
                                                <td class="flex p-5 items-center gap-0.5">
                                                    ...
                                                    <button
                                                        ...
                                                        v-if="$page.props.user.permissions.includes('delete city')"
                ...
                <div class="flex items-center justify-between border-t border-gray-200 px-4 py-3 sm:px-6 mt-4">
                    ...
                    <div class="flex flex-1 justify-between sm:hidden">
                        <Link v-if="cities.prev_page_url"
                            :href="cities.prev_page_url"
                            class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                            Anterior
                        </Link>
                        <Link v-if="cities.next_page_url"
                    ...
                    <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm text-gray-700">
                                Mostrando
                                <span class="font-medium">{{ cities.from }}</span>
                                a
                                <span class="font-medium">{{ cities.to }}</span>
                                de
                        ...
                        <div>
                            <nav aria-label="Pagination" class="isolate inline-flex -space-x-px rounded-md shadow-sm">
                                ...
                                <!-- Page Numbers -->
                                <Link v-for="page in cities.last_page" :key="page"
        ...
        <ConfirmModal 
            ...
            message="¿Estás seguro de que deseas eliminar esta ciudad? Esta acción no se puede deshacer."
