...
<script setup>
    ...
    import CreateModal from '@/Components/HousingType/CreateModal.vue';
    import ViewModal from '@/Components/HousingType/ViewModal.vue';
    import EditModal from '@/Components/HousingType/EditModal.vue';
    ...
    const showHousingTypeModal = ref(false);
    const showEditModal = ref(false);
    const selectedHousingType = ref(null);
    const showDeleteConfirm = ref(false);
    const itemToDelete = ref(null);

    const openHousingTypeModal = (item) => {
        selectedHousingType.value = item;
        showHousingTypeModal.value = true;
    };

    const openHousingTypeModalForEdit = (item) => {
        selectedHousingType.value = item;
        showEditModal.value = true;
    };

    const closeHousingTypeModal = () => {
        showHousingTypeModal.value = false;
        showEditModal.value = false;
        selectedHousingType.value = null;
    };
    ...
    const confirmDelete = () => {
        if (itemToDelete.value) {
            router.delete(route('housingtype.destroy', itemToDelete.value.houTyp_id), {
                onSuccess: () => {
                    showDeleteConfirm.value = false;
                    itemToDelete.value = null;
                    
                    notificationStore.success('Tipo de vivienda eliminado exitosamente');
                ...
                onError: (error) => {
                    console.error('Error deleting housing type:', error);
                    notificationStore.error('Error al eliminar el tipo de vivienda');
                    showDeleteConfirm.value = false;
                    itemToDelete.value = null;
                }
            });
...
<template>
    <AppLayout title="Tipo de Vivienda">
        <HeaderBody titulo-body="Tipos de Vivienda" permisos="create housing type" nombre-btn="Crear nuevo" @create="showCreateModal = true"/>
        ...
        <!-- Paginación -->
        <Pagination :objPage="housing_types" />

        <!-- Modal de Información del Tipo de Vivienda -->
        <ViewModal 
            :show="showHousingTypeModal" 
            :housing-type="selectedHousingType"
            @close="closeHousingTypeModal"
        />

        <!-- Modal de Creación de Tipo de Vivienda -->
        <CreateModal :show="showCreateModal" @close="showCreateModal = false" />

        <!-- Modal de Edición de Tipo de Vivienda -->
        <EditModal
            :show="showEditModal"
            :housing-type="selectedHousingType"
            @close="closeHousingTypeModal"
        ...
        <ConfirmModal 
            ...
            message="¿Estás seguro de que deseas eliminar este tipo de vivienda? Esta acción no se puede deshacer."
