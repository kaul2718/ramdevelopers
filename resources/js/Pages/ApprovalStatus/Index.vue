...
const approvalStatusToDelete = ref(null);

const openApprovalStatusModal = (approvalStatus) => {
...
const openDeleteConfirm = (approvalStatus) => {
    approvalStatusToDelete.value = approvalStatus;
    showDeleteConfirm.value = true;
};

const confirmDelete = () => {
    if (approvalStatusToDelete.value) {
        router.delete(route('approvalstatus.destroy', approvalStatusToDelete.value.apvSta_id), {
            onSuccess: () => {
                showDeleteConfirm.value = false;
                approvalStatusToDelete.value = null;
                
                notificationStore.success('Estado de aprobación eliminado exitosamente');
            ...
            onError: (error) => {
                ...
                notificationStore.error('Error al eliminar el estado de aprobación');
                showDeleteConfirm.value = false;
                approvalStatusToDelete.value = null;
            }
        });
...
<template>
    <AppLayout>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-end pr-2" v-if="$page.props.user.permissions.includes('create approval status')">
                                    ...
                                    <table class="w-full rounded-xl">
                                        <thead>
                                            <tr class="bg-gray-50">
                                                ...
                                                <th scope="col" class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize min-w-[150px]">Código</th>
                                        ...
                                        <tbody class="divide-y divide-gray-300">
                                            <tr v-for="item in approval_statuses.data" :key="item.apvSta_id" class="bg-white transition-all duration-500 hover:bg-gray-50">
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ item.apvSta_id }}</td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ item.apvSta_name }}</td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ item.apvSta_code }}</td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ item.apvSta_description }}</td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    <div :class="['py-1.5 px-2.5 rounded-full flex justify-center w-24 items-center gap-2', item.apvSta_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600']">
                                                        <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <circle cx="3" cy="3" r="3" :fill="item.apvSta_active ? '#059669' : '#dc2626'" />
                                                        </svg>
                                                        <span class="font-semibold text-xs">{{ item.apvSta_active ? 'Activo' : 'Inactivo' }}</span>
                                                    </div>
                                                </td>
                                                <td class="flex p-5 items-center gap-0.5">
                                                    <button
                                                        @click="openApprovalStatusModal(item)"
                                                    ...
                                                    <button
                                                        @click="openApprovalStatusModalForEdit(item)"
                                                        ...
                                                        title="Editar estado de aprobación">
                                                    ...
                                                    <button
                                                        ...
                                                        title="Eliminar estado de aprobación">
        ...
        <ConfirmModal
            ...
            title="Eliminar Estado de Aprobación"
            message="¿Estás seguro de que deseas eliminar este estado de aprobación? Esta acción es irreversible."
