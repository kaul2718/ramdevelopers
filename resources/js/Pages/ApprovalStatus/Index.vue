    ...
    <AppLayout title="Estado Aprobacion">
        ...
        <div class="caja--tabla">
            <table>
                ...
                <tbody>
                    <tr v-for="item in approval_statuses.data" :key="item.apvSta_id" @click="openApprovalStatusModal(item)">
                        <td>{{ item.apvSta_id }}</td>
                        <td>{{ item.apvSta_name }}</td>
                        <td>{{ item.apvSta_code }}</td>
                        <td>{{ item.apvSta_description }}</td>
                        <td>
                            <div :class="['px-2 rounded-md flex justify-center w-fit items-center gap-2', item.apvSta_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600']">
                                <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <circle cx="3" cy="3" r="3" :fill="item.apvSta_active ? '#059669' : '#dc2626'" />
                                </svg>
                                <span>{{ item.apvSta_active ? 'Activo' : 'Inactivo' }}</span>
                            </div>
                        </td>
                        <td class="botonera--tabla">
                            <button @click.stop="openApprovalStatusModalForEdit(item)" class="btn--tipo1" title="Editar campo" v-if="$page.props.user.permissions.includes('update approval status')">
                            ...
                            <button @click.stop="openDeleteConfirm(item)"class="btn--tipo2" title="Eliminar campo" v-if="$page.props.user.permissions.includes('delete approval status')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
