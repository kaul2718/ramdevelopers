...
<script setup>
    ...
    import UserViewModal from '@/Components/User/ViewModal.vue';
    ...
    // Determinar si el usuario actual es Master Pais
    const isMasterCountry = computed(() => {
        return page.props.user?.roles?.some(role => role.name === 'Master Pais') || false;
    });

    // Determinar si el usuario puede crear (Master Pais solo en su país)
    const canCreateUser = computed(() => {
        return page.props.user?.permissions?.includes('create users') || false;
    });

    // Filtrar países disponibles para Master Pais (solo su país)
    const availableCountries = computed(() => {
        if (isMasterCountry.value) {
            return props.countries.filter(c => c.ctry_id === page.props.user.usr_id_ctry);
        }
    ...
    const openUserModalForEdit = (user) => {
        // Master Pais solo puede editar usuarios de su país
        if (isMasterCountry.value && user.usr_id_ctry !== page.props.user.usr_id_ctry) {
            return;
        }
        selectedUser.value = user;
        editMode.value = true;
...
<template>
    <AppLayout title="Usuarios">
        <HeaderBody titulo-body="Gestión de usuarios" permisos="create users" nombre-btn="Crear nuevo" @create="showCreateModal = true"/>

        <div class="caja--tabla">
            <table>
                <thead>
                    <tr>
                        <th>Foto</th><th>Nombre</th><th>Apellido</th><th>Correo</th><th>País</th><th>Rol</th><th>Estado</th><th>Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="item in users.data" :key="item.id" @click="openUserModal(item)">
                        <td>
                            <img v-if="item.profile_photo_path" :src="'/storage/' + item.profile_photo_path" :alt="item.name" class="size-16 rounded-full object-cover">
                            <div v-else class="size-16 rounded-full bg-gray-300 flex items-center justify-center text-sm font-semibold text-gray-700">{{ item.name.charAt(0) }}</div>
                        </td>
                        <td>{{ item.name }}</td>
                        <td>{{ item.lastname }}</td>
                        <td>{{ item.email }}</td>
                        <td>
                            <span v-if="item.country" class="inline-flex items-center rounded-md bg-blue-50 px-2 text-blue-700">
                                {{ item.country.ctry_name }}
                            </span>
                            <span v-else class="text-gray-400">N/A</span>
                        </td>
                        <td>
                            <div v-if="item.roles && item.roles.length > 0" class="flex flex-wrap gap-1">
                                <span v-for="role in item.roles" :key="role.id" class="inline-flex items-center rounded-md bg-purple-50 px-2 text-purple-700">
                                    {{ role.name }}
                                </span>
                            </div>
                            <span v-else class="text-gray-400">Sin roles</span>
                        </td>
                        <td>
                            <div :class="['px-2 rounded-md flex justify-center w-fit items-center gap-2', item.usr_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600']">
                                <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <circle cx="3" cy="3" r="3" :fill="item.usr_active ? '#059669' : '#dc2626'" />
                                </svg>
                                <span>{{ item.usr_active ? 'Activo' : 'Inactivo' }}</span>
                            </div>
                        </td>
                        <td>
                            <div class="botonera--tabla">
                                <button @click.stop="openUserModalForEdit(item)" class="btn--tipo1" title="Editar campo" v-if="$page.props.user.permissions.includes('update users') && (!isMasterCountry || item.usr_id_ctry === $page.props.user.usr_id_ctry)">
                                ...
                                <button @click.stop="openDeleteConfirm(item.id)" class="btn--tipo2" title="Eliminar campo" v-if="$page.props.user.permissions.includes('delete users') && item.id !== $page.props.user.id">
                                    <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
