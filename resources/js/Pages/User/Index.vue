<!-- filepath: resources/js/Pages/User/Index.vue -->
<script>
    export default {
        name: 'UserIndex'
    }
...
<script setup>
    ...
    onMounted(() => {
        ...
        if (sessionStorage.getItem('showCreateNotification')) {
            setTimeout(() => {
                notificationStore.success('Usuario creado exitosamente');
                sessionStorage.removeItem('showCreateNotification');
    ...
    <AppLayout title="Usuarios">
        ...
        <div class="caja--tabla">
            <table>
                <thead>
                    <tr>
                        <th>Foto</th><th>Nombre</th><th>Apellido</th><th>Correo</th><th>País</th><th>Roles</th><th>Estado</th><th>Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="item in users.data" :key="item.id">
                        <td>
                            <img v-if="item.profile_photo_url" :src="item.profile_photo_url" :alt="item.name" class="w-10 h-10 rounded-full object-cover">
                            <div v-else class="w-10 h-10 rounded-full bg-gray-300 flex items-center justify-center text-sm font-semibold text-gray-700">{{ item.name.charAt(0) }}</div>
                        </td>
                        <td>{{ item.name }}</td>
                        <td>{{ item.lastname }}</td>
                        <td>{{ item.email }}</td>
                        <td>
                            <span v-if="item.country" class="inline-flex items-center rounded-md bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700">
                        ...
                        <td>
                            <div v-if="item.roles && item.roles.length > 0" class="flex flex-wrap gap-1">
                                <span v-for="role in item.roles" :key="role.id" class="inline-flex items-center rounded-md bg-purple-50 px-2 py-1 text-xs font-medium text-purple-700">
                                    {{ role.name }}
                                </span>
                            </div>
                        ...
                        <td>
                            <div :class="['py-1.5 px-2.5 rounded-full flex justify-center w-24 items-center gap-2', item.usr_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600']">
                                <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <circle cx="3" cy="3" r="3" :fill="item.usr_active ? '#059669' : '#dc2626'" />
                                </svg>
                                <span class="font-semibold text-xs">{{ item.usr_active ? 'Activo' : 'Inactivo' }}</span>
                            </div>
                        </td>
                        <td class="botonera--tabla">
                            <button @click="openUserModal(item)" class="btn--tipo1" title="Ver información">
                            ...
                            <button @click="openUserModalForEdit(item)" class="btn--tipo1" title="Editar campo" v-if="$page.props.user.permissions.includes('update users')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <button @click="openDeleteConfirm(item.id)"class="btn--tipo2" title="Eliminar campo" v-if="$page.props.user.permissions.includes('delete users')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
