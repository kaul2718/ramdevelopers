...
<template>
    <AppLayout title="Crear Rol">
        <template #header>
            <h1 class="font-semibold text-2xl text-gray-800">Crear Nuevo Rol</h1>
        </template>
        ...
        <div v-if="!isDevAdmin()" class="py-12">
            <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
                <div class="p-6 bg-red-50 border border-red-200 rounded-lg">
                    <p class="text-red-800">⚠️ No tienes permiso para acceder a esta sección. Solo DevAdmin puede crear roles.</p>
                </div>
            </div>
        </div>
            ...
            <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white rounded-lg shadow-md overflow-hidden">
                    ...
                    <div class="bg-gradient-to-r from-indigo-500 to-indigo-600 px-6 py-4">
                        ...
                        <p class="text-indigo-100 text-sm mt-1">Completa los datos para crear un nuevo rol del sistema</p>
                    </div>
                    ...
                    <form @submit.prevent="form.post(route('roles.store'))" class="p-6">
                        ...
                        <div class="mb-6">
                            <label class="block text-sm font-semibold text-gray-900 mb-3">
                                Asignar Permisos <span class="text-gray-500 text-xs font-normal">(Opcional)</span>
                            </label>
                            <div class="bg-gray-50 rounded-lg p-4">
                                <div v-if="permissions && permissions.length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <label v-for="p in permissions" :key="p.id" class="flex items-start gap-3 p-2 hover:bg-white rounded-lg transition-colors cursor-pointer">
                                        <input 
                                            type="checkbox" 
                                            :value="p.id" 
