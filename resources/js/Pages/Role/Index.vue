        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="p-6 bg-white border-border-gray-200">
                    <div class="flex justify-end pr-2" v-if="$page.props.user?.permissions?.includes('create roles')">
                        <Link 
                                    ...
                                    <table class="w-full rounded-xl">
                                        <thead>
                                            <tr class="bg-gray-50">
                                                ...
                                                <th scope="col" class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize min-w-[150px]">Nombre</th>
                                                <th scope="col" class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize">Permisos</th>
                                        ...
                                        <tbody class="divide-y divide-gray-300">
                                            <tr v-for="item in roles.data" :key="item.id" class="bg-white transition-all duration-500 hover:bg-gray-50">
                                                ...
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    <div v-if="item.permissions && item.permissions.length > 0" class="flex flex-wrap gap-1">
                                                        <span v-for="perm in item.permissions" :key="perm.id" class="inline-flex items-center rounded-md bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700">{{ perm.name }}</span>
                                                    </div>
                                                    <span v-else class="text-gray-400">Sin permisos</span>
                                                </td>
                                                <td class="flex p-5 items-center gap-0.5">
                                                    ...
                                                    <Link @click.prevent="deleteRole(item.id)"
                ...
                <div class="flex items-center justify-between border-t border-gray-200 px-4 py-3 sm:px-6 mt-4">
                    ...
                    <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm text-gray-700">Mostrando <span class="font-medium">{{ roles.from }}</span> a <span class="font-medium">{{ roles.to }}</span> de <span class="font-medium">{{ roles.total }}</span> resultados</p>
                        </div>
