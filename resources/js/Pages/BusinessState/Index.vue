...
const showBusinessStateModal = ref(false);
const selectedBusinessState = ref(null);
const editMode = ref(false);
const showDeleteConfirm = ref(false);
const businessStateToDelete = ref(null);

const openBusinessStateModal = (businessState) => {
    selectedBusinessState.value = businessState;
    editMode.value = false;
    showBusinessStateModal.value = true;
};

const openBusinessStateModalForEdit = (businessState) => {
    selectedBusinessState.value = businessState;
    editMode.value = true;
    showBusinessStateModal.value = true;
};
...
const closeBusinessStateModal = () => {
    showBusinessStateModal.value = false;
...
<template>
    <AppLayout>
        <template #header>
            <h1 class="font-semibold text-xl text-gray-800 leading-tight">Estado de Negocio</h1>
        </template>
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="p-6 bg-white border-border-gray-200">
                    <div class="flex justify-end pr-2"
                        v-if="$page.props.user.permissions.includes('create business state')">
                            ...
                            <div class="block">
                                    ...
                                    <table class="w-full rounded-xl">
                                        ...
                                        <tbody class="divide-y divide-gray-300 ">
                                            <tr v-for="item in business_states.data" :key="item.busiSta_id"
                                                ...
                                                <td
                                                    class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    {{ item.busiSta_description }}
                                                </td>
                                                <td
                                                    class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    <div :class="[
                                                        'py-1.5 px-2.5 rounded-full flex justify-center w-24 items-center gap-2',
                                                        item.busiSta_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'
                                                    ]">
                                                        <svg width="6" height="6" viewBox="0 0 6 6" fill="none"
                                                            xmlns="http://www.w3.org/2000/svg">
                                                            <circle cx="3" cy="3" r="3"
                                                                :fill="item.busiSta_active ? '#059669' : '#dc2626'" />
                                                        </svg>

                                                        <span class="font-semibold text-xs">
                                                            {{ item.busiSta_active ? 'Activo' : 'Inactivo' }}
                                                        </span>
                                                    </div>
                                                </td>
                                                ...
                                                <td class="flex p-5 items-center gap-0.5">
                                                    <button
                                                        @click="openBusinessStateModal(item)"
                                                        v-if="$page.props.user.permissions.includes('read business state')"
                                                    ...
                                                    <button
                                                        @click="openBusinessStateModalForEdit(item)"
                                ...
                                <div
                                    ...
                                    <div class="flex flex-1 justify-between sm:hidden">
                                        <Link v-if="business_states.prev_page_url" :href="business_states.prev_page_url"
                                            class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                                            Anterior
                                        </Link>
                                        <Link v-if="business_states.next_page_url" :href="business_states.next_page_url"
                                    ...
                                    <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                                        <div>
                                            <p class="text-sm text-gray-700">
                                                Mostrando
                                                <span class="font-medium">{{ business_states.from }}</span>
                                                a
                                                <span class="font-medium">{{ business_states.to }}</span>
                                                de
                                                <span class="font-medium">{{ business_states.total }}</span>
                                                resultados
                                            </p>
                                        </div>
                                        ...
                                        <div>
                                            <nav aria-label="Pagination"
                                                ...
                                                <!-- Previous Button -->
                                                <Link v-if="business_states.prev_page_url"
                                                    :href="business_states.prev_page_url"
                                                ...
                                                <!-- Page Numbers -->
                                                <Link v-for="page in business_states.last_page" :key="page"
                                                    :href="`?page=${page}`" :class="[
                                                        'relative inline-flex items-center px-4 py-2 text-sm font-semibold focus:z-20 focus:outline-offset-0',
                                                        page === business_states.current_page
                                                ...
                                                <!-- Next Button -->
                                                <Link v-if="business_states.next_page_url"
                                                    :href="business_states.next_page_url"
        ...
        <!-- ViewModal -->
        <ViewModal
            :show="showBusinessStateModal"
            :businessState="selectedBusinessState"
            :editMode="editMode"
            @close="closeBusinessStateModal"
        />
