...
<template>
    <AppLayout title="Tipos documentos">
        <HeaderBody titulo-body="Tipos de Documento" permisos="create document types" nombre-btn="Crear nuevo" @create="showCreateModal = true"/>
        ...
        <div class="caja--tabla">
            <table>
                ...
                <tbody>
                    <tr v-for="item in document_types.data" :key="item.docTyp_id"  @click="openDocumentTypeModal(item)">
                        ...
                        <td>
                            <div :class="['px-2 rounded-md flex justify-center w-fit items-center gap-2', item.docTyp_isInternal ? 'bg-blue-50 text-blue-600' : 'bg-gray-50 text-gray-600']">
                                ...
                                <span>{{ item.docTyp_isInternal ? 'Sí' : 'No' }}</span>
                            </div>
                        </td>
                        <td>
                            <div :class="['px-2 rounded-md flex justify-center w-fit items-center gap-2', item.docTyp_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600']">
                                ...
                                <span>{{ item.docTyp_active ? 'Activo' : 'Inactivo' }}</span>
                            </div>
                        </td>
                        <td class="botonera--tabla">
                            <button @click.stop="openDocumentTypeModalForEdit(item)" class="btn--tipo1" title="Editar campo" v-if="$page.props.user.permissions.includes('update document types')">
                            ...
                            <button @click.stop="openDeleteConfirm(item)"class="btn--tipo2" title="Eliminar campo" v-if="$page.props.user.permissions.includes('delete document types')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
