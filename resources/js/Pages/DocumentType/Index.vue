<!-- filepath: resources/js/Pages/DocumentType/Index.vue -->
<script>
export default {
...
import CreateModal from '@/Components/DocumentType/CreateModal.vue';
import ViewModal from '@/Components/DocumentType/ViewModal.vue';
...
const openDocumentTypeModal = (documentType) => {
    selectedDocumentType.value = documentType;
    editMode.value = false;
    showDocumentTypeModal.value = true;
};

const openDocumentTypeModalForEdit = (documentType) => {
    selectedDocumentType.value = documentType;
    editMode.value = true;
    showDocumentTypeModal.value = true;
};
...
const openDeleteConfirm = (documentType) => {
...
const confirmDelete = () => {
    if (documentTypeToDelete.value) {
        router.delete(route('documenttype.destroy', documentTypeToDelete.value.docTyp_id), {
            onSuccess: () => {
                showDeleteConfirm.value = false;
                documentTypeToDelete.value = null;
                
                // Mostrar notificación inmediatamente
                notificationStore.success('Tipo de documento eliminado exitosamente');
            ...
            onError: (error) => {
                ...
                notificationStore.error('Error al eliminar el tipo de documento');
                showDeleteConfirm.value = false;
                documentTypeToDelete.value = null;
            }
        });
...
onMounted(() => {
    ...
    if (sessionStorage.getItem('showCreateDocumentTypeNotification')) {
        setTimeout(() => {
            notificationStore.success('Tipo de documento creado exitosamente');
            sessionStorage.removeItem('showCreateDocumentTypeNotification');
    ...
    if (sessionStorage.getItem('showUpdateDocumentTypeNotification')) {
        setTimeout(() => {
            notificationStore.success('Tipo de documento actualizado exitosamente');
            sessionStorage.removeItem('showUpdateDocumentTypeNotification');
...
<template>
    <AppLayout>
        <template #header>
            <h1 class="font-semibold text-xl text-gray-800 leading-tight">Tipo de Documento</h1>
        </template>

        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="p-6 bg-white border-border-gray-200">
                    <div class="flex justify-end pr-2" v-if="$page.props.user.permissions.includes('create document types')">
                        <button 
                                ...
                                <div class="overflow-x-auto w-full border rounded-lg border-gray-300">
                                    <table class="w-full rounded-xl">
                                        ...
                                        <tbody class="divide-y divide-gray-300">
                                            <tr 
                                                v-for="item in document_types.data" 
                                                :key="item.docTyp_id"
                                                ...
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    {{ item.docTyp_description }}
                                                </td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    <div :class="[
                                                        'py-1.5 px-2.5 rounded-full flex justify-center w-16 items-center gap-2',
                                                        item.docTyp_isInternal ? 'bg-blue-50 text-blue-600' : 'bg-gray-50 text-gray-600'
                                                    ]">
                                                        <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <circle cx="3" cy="3" r="3" :fill="item.docTyp_isInternal ? '#2563eb' : '#9ca3af'" />
                                                        </svg>
                                                        <span class="font-semibold text-xs">
                                                            {{ item.docTyp_isInternal ? 'Sí' : 'No' }}
                                                        </span>
                                                    </div>
                                                </td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    <div :class="[
                                                        ...
                                                        item.docTyp_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'
                                                    ]">
                                                        <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <circle cx="3" cy="3" r="3" :fill="item.docTyp_active ? '#059669' : '#dc2626'" />
                                                        </svg>
                                                        <span class="font-semibold text-xs">
                                                            {{ item.docTyp_active ? 'Activo' : 'Inactivo' }}
                                                        </span>
                                                    </div>
                                                </td>
                                                <td class="flex p-5 items-center gap-0.5">
                                                    <button
                                                        @click="openDocumentTypeModal(item)"
                                                        v-if="$page.props.user.permissions.includes('read document types')"
                ...
                <div class="flex items-center justify-between border-t border-gray-200 px-4 py-3 sm:px-6 mt-4">
                    ...
                    <div class="flex flex-1 justify-between sm:hidden">
                        <Link v-if="document_types.prev_page_url"
                            :href="document_types.prev_page_url"
                            class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                            Anterior
                        </Link>
                        <Link v-if="document_types.next_page_url"
                            :href="document_types.next_page_url"
                    ...
                    <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm text-gray-700">
                                Mostrando
                                <span class="font-medium">{{ document_types.from }}</span>
                                a
                                <span class="font-medium">{{ document_types.to }}</span>
                                de
                                <span class="font-medium">{{ document_types.total }}</span>
                                resultados
                            </p>
                        </div>
                        ...
                        <div>
                            <nav aria-label="Pagination" class="isolate inline-flex -space-x-px rounded-md shadow-sm">
                                ...
                                <!-- Page Numbers -->
                                <Link v-for="page in document_types.last_page" :key="page"
                                    :href="`?page=${page}`" :class="[
                                        'relative inline-flex items-center px-4 py-2 text-sm font-semibold focus:z-20 focus:outline-offset-0',
                                        page === document_types.current_page
        ...
        <ConfirmModal
            ...
            message="¿Estás seguro de que deseas eliminar este tipo de documento? Esta acción no se puede deshacer."
