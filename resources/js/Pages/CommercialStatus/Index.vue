...
import CreateModal from '@/Components/CommercialStatus/CreateModal.vue';
import ViewModal from '@/Components/CommercialStatus/ViewModal.vue';
...
const showCommercialStatusModal = ref(false);
const selectedCommercialStatus = ref(null);
...
const openCommercialStatusModal = (item) => {
    selectedCommercialStatus.value = item;
    editMode.value = false;
    showCommercialStatusModal.value = true;
};

const openCommercialStatusModalForEdit = (item) => {
    selectedCommercialStatus.value = item;
    editMode.value = true;
    showCommercialStatusModal.value = true;
};

const closeCommercialStatusModal = () => {
    showCommercialStatusModal.value = false;
    editMode.value = false;
    selectedCommercialStatus.value = null;
};
...
const confirmDelete = () => {
    if (itemToDelete.value) {
        router.delete(route('commercialstatus.destroy', itemToDelete.value.commSta_id), {
            onSuccess: () => {
                showDeleteConfirm.value = false;
                itemToDelete.value = null;
...
<template>
    <AppLayout>
        <template #header>
            <h1 class="font-semibold text-xl text-gray-800 leading-tight">Estado Comercial</h1>
        </template>
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="p-6 bg-white border-border-gray-200">
                    <div class="flex justify-end pr-2"
                        v-if="$page.props.user.permissions.includes('create commercial status')">
                            ...
                            <div class="block">
                                <div class="overflow-x-auto w-full  border rounded-lg border-gray-300">
                                    <table class="w-full rounded-xl">
                                        ...
                                        <tbody class="divide-y divide-gray-300 ">
                                            <tr v-for="item in commercial_statuses.data" :key="item.commSta_id"
                                                ...
                                                <td
                                                    class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    {{ item.commSta_description }}
                                                </td>
                                                <td
                                                    class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    <div :class="[
                                                        'py-1.5 px-2.5 rounded-full flex justify-center w-24 items-center gap-2',
                                                        item.commSta_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'
                                                    ]">
                                                        <svg width="6" height="6" viewBox="0 0 6 6" fill="none"
                                                            xmlns="http://www.w3.org/2000/svg">
                                                            <circle cx="3" cy="3" r="3"
                                                                :fill="item.commSta_active ? '#059669' : '#dc2626'" />
                                                        </svg>

                                                        <span class="font-semibold text-xs">
                                                            {{ item.commSta_active ? 'Activo' : 'Inactivo' }}
                                                        </span>
                                                    </div>
                                                </td>

                                                <td class="flex p-5 items-center gap-0.5">
                                                    <button @click="openCommercialStatusModal(item)"
                                                        ...
                                                        <svg class="cursor-pointer" width="20" height="20"
                                                            ...
                                                            <path class="fill-blue-600 group-hover:fill-white"
                                                                d="M10.0161 3.39502C5.50504 3.39502 2.73527 6.41251 2.05457 8.80592C1.80339 9.62704 1.80339 10.373 2.05457 11.1941C2.73527 13.5875 5.50504 16.605 10.0161 16.605C14.527 16.605 17.2968 13.5875 17.9775 11.1941C18.2287 10.373 18.2287 9.62704 17.9775 8.80592C17.2968 6.41251 14.527 3.39502 10.0161 3.39502ZM10.0161 14.3034C7.83379 14.3034 6.07506 12.5447 6.07506 10.3624C6.07506 8.18009 7.83379 6.42136 10.0161 6.42136C12.1984 6.42136 13.9572 8.18009 13.9572 10.3624C13.9572 12.5447 12.1984 14.3034 10.0161 14.3034ZM10.0161 7.88959C8.76924 7.88959 7.74329 8.91555 7.74329 10.1624C7.74329 11.4093 8.76924 12.4352 10.0161 12.4352C11.263 12.4352 12.289 11.4093 12.289 10.1624C12.289 8.91555 11.263 7.88959 10.0161 7.88959Z"
                                                                fill="#2563eb"></path>
                                                        </svg>
                                                    </button>
                                                    <button @click="openCommercialStatusModalForEdit(item)"
                                                    ...
                                                    <button @click="openDeleteConfirm(item)"
                                                        v-if="$page.props.user.permissions.includes('delete commercial status')"
                                ...
                                <div class="flex items-center justify-between border-t border-gray-200 px-4 py-3 sm:px-6 mt-4">
                                    ...
                                    <div class="flex flex-1 justify-between sm:hidden">
                                        <Link v-if="commercial_statuses.prev_page_url"
                                            :href="commercial_statuses.prev_page_url"
                                            class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                                            Anterior
                                        </Link>
                                        <Link v-if="commercial_statuses.next_page_url"
                                            :href="commercial_statuses.next_page_url"
                                    ...
                                    <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                                        <div>
                                            <p class="text-sm text-gray-700">
                                                Mostrando
                                                <span class="font-medium">{{ commercial_statuses.from }}</span>
                                                a
                                                <span class="font-medium">{{ commercial_statuses.to }}</span>
                                                de
                                                <span class="font-medium">{{ commercial_statuses.total }}</span>
                                                resultados
                                            </p>
                                        </div>
                                        ...
                                        <div>
                                            <nav aria-label="Pagination" class="isolate inline-flex -space-x-px rounded-md shadow-sm">
                                                ...
                                                <!-- Page Numbers -->
                                                <Link v-for="page in commercial_statuses.last_page" :key="page"
                                                    :href="`?page=${page}`" :class="[
                                                        'relative inline-flex items-center px-4 py-2 text-sm font-semibold focus:z-20 focus:outline-offset-0',
                                                        page === commercial_statuses.current_page
        ...
        <!-- Modales -->
        <CreateModal :show="showCreateModal" @close="closeCreateModal"></CreateModal>
        <ViewModal :show="showCommercialStatusModal" :commercial-status="selectedCommercialStatus" :edit-mode="editMode" @close="closeCommercialStatusModal"></ViewModal>
