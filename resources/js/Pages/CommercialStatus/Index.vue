...
<script setup>
    ...
    import EditModal from '@/Components/CommercialStatus/EditModal.vue';
...
<template>
    <AppLayout title="Tipo de Proyecto">
            ...
            <table>
                ...
                <tbody>
                    <tr v-for="item in commercial_statuses.data" :key="item.commSta_id">
                        <td>{{ item.commSta_id }}</td>
                        <td>{{ item.commSta_name }}</td>
                        <td>{{ item.commSta_code }}</td>
                        <td>{{ item.commSta_description }}</td>
                        <td>
                            <div :class="['py-1.5 px-2.5 rounded-full flex justify-center w-24 items-center gap-2', item.commSta_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600']">
                                <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <circle cx="3" cy="3" r="3" :fill="item.commSta_active ? '#059669' : '#dc2626'" />
                                </svg>
                                <span class="font-semibold text-xs">{{ item.commSta_active ? 'Activo' : 'Inactivo' }}</span>
                            </div>
                        </td>
                        <td class="botonera--tabla">
                            <button @click="openCommercialStatusModal(item)" class="btn--tipo1" title="Ver información">
                            ...
                            <button @click="openCommercialStatusModalForEdit(item)" class="btn--tipo1" title="Editar campo" v-if="$page.props.user.permissions.includes('update commercial status')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <button @click="openDeleteConfirm(item)"class="btn--tipo2" title="Eliminar campo" v-if="$page.props.user.permissions.includes('delete commercial status')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Paginación -->
        <Pagination :objPage="commercial_statuses" />

        <!-- Modal de Información del Tipo de Proyecto -->
        <ViewModal 
            :show="showCommercialStatusModal" 
            :commercial-status="selectedCommercialStatus"
            @close="closeCommercialStatusModal"
        />

        <!-- Modal de Creación de Tipo de Proyecto -->
        <CreateModal :show="showCreateModal" @close="showCreateModal = false" />

        <!-- Modal de Edición de Tipo de Proyecto -->
        <EditModal
            :show="showEditModal"
            :commercial-status="selectedCommercialStatus"
            @close="closeCommercialStatusModal"
