...
<template>
    <AppLayout title="Promotores">
        <HeaderBody titulo-body="Gestionar Promotores" permisos="create developer" nombre-btn="Crear nuevo" @create="showCreateModal = true"/>
        ...
        <div class="caja--tabla">
            <table>
                <thead>
                    <tr>
                        <th>Id</th><th>Nombre Comercial</th><th>Nombre Legal</th><th>Email</th><th>País</th><th>Usuario</th><th>Estado</th><th>Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="item in developers.data" :key="item.devr_id">
                        ...
                        <td>
                            <div :class="['py-1.5 px-2.5 rounded-full flex justify-center w-24 items-center gap-2', item.devr_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600']">
                                <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <circle cx="3" cy="3" r="3" :fill="item.devr_active ? '#059669' : '#dc2626'" />
                                </svg>
                                <span class="font-semibold text-xs">{{ item.devr_active ? 'Activo' : 'Inactivo' }}</span>
                            </div>
                        </td>
                        <td class="botonera--tabla">
                            <button @click="openDeveloperModalForView(item)" class="btn--tipo1" title="Ver información" v-if="$page.props.user.permissions.includes('read developer')">
                            ...
                            <button @click="openDeveloperModalForEdit(item)" class="btn--tipo1" title="Editar campo" v-if="$page.props.user.permissions.includes('update developer')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <button @click="openDeleteConfirm(item)"class="btn--tipo2" title="Eliminar campo" v-if="$page.props.user.permissions.includes('delete developer')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
