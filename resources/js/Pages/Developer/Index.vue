        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="p-6 bg-white border-border-gray-200">
                    ...
                    <div class="flex justify-end pr-2" v-if="$page.props.user.permissions.includes('create developer')">
                        <button 
                                    ...
                                    <table class="w-full rounded-xl">
                                        ...
                                        <tbody class="divide-y divide-gray-300">
                                            <tr 
                                                ...
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    {{ item.devr_commercial_name }}
                                                </td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    {{ item.devr_legal_name }}
                                                </td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    <a :href="`mailto:${item.devr_email_contact}`" class="text-indigo-600 hover:text-indigo-900">
                                                ...
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    <span class="inline-flex items-center rounded-md bg-green-50 px-2 py-1 text-xs font-medium text-green-700">
                                                        {{ item.user?.name || 'Sin usuario' }}
                                                    </span>
                                                </td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    <div :class="[
                                                        'py-1.5 px-2.5 rounded-full flex justify-center w-24 items-center gap-2',
                                                        item.devr_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'
                                                    ]">
                                                        <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <circle cx="3" cy="3" r="3" :fill="item.devr_active ? '#059669' : '#dc2626'" />
                                                        </svg>
                                                        <span class="font-semibold text-xs">
                                                            {{ item.devr_active ? 'Activo' : 'Inactivo' }}
                                                        </span>
                                                    </div>
                                                </td>
                                                <td class="flex p-5 items-center gap-0.5">
                                                    <button
                                                        @click="openDeveloperModalForView(item)"
                                                    ...
                                                    <button
                                                        @click="openDeveloperModalForEdit(item)"
                ...
                <div class="flex items-center justify-between border-t border-gray-200 px-4 py-3 sm:px-6 mt-4">
                    ...
                    <div class="flex flex-1 justify-between sm:hidden">
                        <Link v-if="developers.prev_page_url"
                            :href="developers.prev_page_url"
                            class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                            Anterior
                        </Link>
                        <div v-else class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-500 cursor-not-allowed">
                            Anterior
                        </div>

                        <Link v-if="developers.next_page_url"
                            :href="developers.next_page_url"
                            class="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                            Siguiente
                        </Link>
                        <div v-else class="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-500 cursor-not-allowed">
                            Siguiente
                        </div>
                    </div>
                    ...
                    <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm text-gray-700">
                                Mostrando
                                <span class="font-medium">{{ developers.from }}</span>
                                a
                                <span class="font-medium">{{ developers.to }}</span>
                                de
                                <span class="font-medium">{{ developers.total }}</span>
                                resultados
                            </p>
                        </div>
                        <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                            ...
                            <div v-else class="relative inline-flex items-center px-2 py-2 text-gray-400 ring-1 ring-inset ring-gray-300 rounded-l-md cursor-not-allowed">
                                <svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                    <path fill-rule="evenodd" d="M12.79 5.23a.75.75 0 01-.02 1.06L8.832 10l3.938 3.71a.75.75 0 11-1.04 1.08l-4.5-4.25a.75.75 0 010-1.08l4.5-4.25a.75.75 0 011.06.02Z" clip-rule="evenodd" />
                                </svg>
                            </div>
                            ...
                            <div v-else class="relative inline-flex items-center px-2 py-2 text-gray-400 ring-1 ring-inset ring-gray-300 rounded-r-md cursor-not-allowed">
                                <svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                    <path fill-rule="evenodd" d="M7.21 14.77a.75.75 0 01.02-1.06L11.168 10 7.23 6.29a.75.75 0 111.04-1.08l4.5 4.25a.75.75 0 010 1.08l-4.5 4.25a.75.75 0 01-1.06-.02Z" clip-rule="evenodd" />
                                </svg>
                            </div>
