    ...
    <AppLayout title="Origen de lead">
        ...
        <div class="caja--tabla">
            <table>
                ...
                <tbody>
                    <tr v-for="item in lead_sources.data" :key="item.leadSou_id" @click="openLeadSourceModal(item)">
                        <td>{{ item.leadSou_id }}</td>
                        <td>{{ item.leadSou_name }}</td>
                        <td>{{ item.leadSou_code }}</td>
                        <td>{{ item.leadSou_description }}</td>
                        <td>
                            <div :class="['px-2 rounded-full flex justify-center w-24 items-center gap-2', item.leadSou_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600']">
                                <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <circle cx="3" cy="3" r="3" :fill="item.leadSou_active ? '#059669' : '#dc2626'" />
                                </svg>
                                <span>{{ item.leadSou_active ? 'Activo' : 'Inactivo' }}</span>
                            </div>
                        </td>
                        <td class="botonera--tabla">
                            <button @click.stop="openLeadSourceModalForEdit(item)" class="btn--tipo1" title="Editar campo" v-if="$page.props.user.permissions.includes('update lead sources')">
                            ...
                            <button @click.stop="openDeleteConfirm(item)"class="btn--tipo2" title="Eliminar campo" v-if="$page.props.user.permissions.includes('delete lead sources')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
