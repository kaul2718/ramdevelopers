...
<script setup>
    ...
    import CreateModal from '@/Components/LeadSource/CreateModal.vue';
    import ViewModal from '@/Components/LeadSource/ViewModal.vue';
    ...
    const leadSourceToDelete = ref(null);

    const openLeadSourceModal = (leadSource) => {
        selectedLeadSource.value = leadSource;
        editMode.value = false;
        showLeadSourceModal.value = true;
    };

    const openLeadSourceModalForEdit = (leadSource) => {
        selectedLeadSource.value = leadSource;
        editMode.value = true;
        showLeadSourceModal.value = true;
    };
    ...
    const openDeleteConfirm = (leadSource) => {
        leadSourceToDelete.value = leadSource;
        showDeleteConfirm.value = true;
    };

    const confirmDelete = () => {
        if (leadSourceToDelete.value) {
            router.delete(route('leadsources.destroy', leadSourceToDelete.value.leadSou_id), {
                onSuccess: () => {
                    showDeleteConfirm.value = false;
                    leadSourceToDelete.value = null;
                    
                    notificationStore.success('Fuente de lead eliminada exitosamente');
                ...
                onError: (error) => {
                    ...
                    notificationStore.error('Error al eliminar la fuente de lead');
                    showDeleteConfirm.value = false;
                    leadSourceToDelete.value = null;
                }
            });
    ...
    onMounted(() => {
        if (sessionStorage.getItem('showCreateLeadSourceNotification')) {
            setTimeout(() => {
                notificationStore.success('Fuente de lead creada exitosamente');
                sessionStorage.removeItem('showCreateLeadSourceNotification');
        ...
        if (sessionStorage.getItem('showUpdateLeadSourceNotification')) {
            setTimeout(() => {
                notificationStore.success('Fuente de lead actualizada exitosamente');
                sessionStorage.removeItem('showUpdateLeadSourceNotification');
...
<template>
    <AppLayout title="Origen de lead">
        <HeaderBody titulo-body="Origenes de leads" permisos="create lead sources" nombre-btn="Crear nuevo" @create="showCreateModal = true"/>

        <div class="caja--tabla">
            <table>
                <thead>
                    <tr>
                        <th>Id</th><th>Nombre</th><th>Código</th><th>Descripción</th><th>Estado</th><th>Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="item in lead_sources.data" :key="item.leadSou_id">
                        <td>{{ item.leadSou_id }}</td>
                        <td>{{ item.leadSou_name }}</td>
                        <td>{{ item.leadSou_code }}</td>
                        <td>{{ item.leadSou_description }}</td>
                        <td>
                            <div :class="['py-1.5 px-2.5 rounded-full flex justify-center w-24 items-center gap-2', item.leadSou_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600']">
                                <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <circle cx="3" cy="3" r="3" :fill="item.leadSou_active ? '#059669' : '#dc2626'" />
                                </svg>
                                <span class="font-semibold text-xs">{{ item.leadSou_active ? 'Activo' : 'Inactivo' }}</span>
                            </div>
                        </td>
                        <td class="botonera--tabla">
                            <button @click="openLeadSourceModal(item)" class="btn--tipo1" title="Ver información">
                            ...
                            <button @click="openLeadSourceModalForEdit(item)" class="btn--tipo1" title="Editar campo" v-if="$page.props.user.permissions.includes('update lead sources')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <button @click="openDeleteConfirm(item)"class="btn--tipo2" title="Eliminar campo" v-if="$page.props.user.permissions.includes('delete lead sources')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Paginación -->
        <Pagination :objPage="lead_sources" />
        ...
        <ViewModal 
            ...
            :edit-mode="editMode"
            @close="closeLeadSourceModal"
        />
        <ConfirmModal
            :show="showDeleteConfirm"
            title="Eliminar Fuente de Lead"
            message="¿Estás seguro de que deseas eliminar esta fuente de lead? Esta acción es irreversible."
