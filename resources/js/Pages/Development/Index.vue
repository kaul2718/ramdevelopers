<script setup>
    ...
    import DevelopmentEditModal from '@/Components/Development/EditModal.vue'
    ...
    const developmentToDelete = ref(null)
    ...
    // Estado de filtro para Agente Inmobiliario
    const filterMode = ref('all') // 'all' para todos, 'personal' para los suyos
    
    const page = usePage();

    // Función para verificar si es Agente Inmobiliario
    const isUserAgenteInmobiliario = () => {
        const isAgent = page.props.user.roles && page.props.user.roles.includes('Agente Inmobiliario');
        return isAgent;
    };

    // Función para verificar si un desarrollo es del usuario
    const isDevelopmentMine = (development) => {
        const isMine = development.user_id === page.props.user.id;
        return isMine;
    };
    ...
    const filteredDevelopments = computed(() => {
        ...
        if (!isUserAgenteInmobiliario() || filterMode.value === 'all') {
            return props.developments;
        }
        
        // Modo 'personal': mostrar solo los del usuario
        const allDevs = props.developments.data || [];
            
        const filtered = {
            ...props.developments,
            data: allDevs.filter(dev => isDevelopmentMine(dev))
        };
    ...
    // Flujo secuencial de modales para CREATE
    ...
    // Flujo secuencial de modales para EDIT
    const handleDevelopmentEdited = (development) => {
        currentDevelopment.value = development
        closeEditModal()
        // Mostrar modal de archivos
        setTimeout(() => {
    ...
    const handleImagesCompleted = () => {
        ...
        notificationStore.success('Cambios guardados exitosamente')
    ...
    const deleteDevelopment = (development) => {
        developmentToDelete.value = development;
        showDeleteConfirm.value = true;
    };

    const confirmDelete = () => {
        if (developmentToDelete.value) {
            router.delete(route('development.destroy', developmentToDelete.value.devt_id), {
                onSuccess: () => {
                    showDeleteConfirm.value = false;
                    developmentToDelete.value = null;

                    notificationStore.success('Desarrollo eliminado exitosamente');
                ...
                onError: (error) => {
                    ...
                    notificationStore.error('Error al eliminar el desarrollo');
                    showDeleteConfirm.value = false;
                    developmentToDelete.value = null;
                }
            });
...
<template>
    <AppLayout title="Proyectos">
        ...
        <!-- Filtro para Agente Inmobiliario -->
        <div v-if="isUserAgenteInmobiliario()" class="mb-4 px-4 py-3 bg-white rounded-lg shadow">
            <label class="block text-sm font-medium text-gray-700 mb-2">Filtrar Proyectos:</label>
            <div class="flex gap-4">
                <label class="flex items-center cursor-pointer">
                    ...
                    >
                    <span class="ml-2 text-sm text-gray-700">Todos los proyectos</span>
                </label>
                <label class="flex items-center cursor-pointer">
                    <input 
                        v-model="filterMode" 
        ...
        <div class="caja--tabla">
            <table>
                ...
                <tbody>
                    <tr v-for="item in filteredDevelopments.data" :key="item.devt_id" @click="goToShow(item.devt_id)">
                        <td style="padding: 0;">
                            <div v-if="item.images && getGalleryImages(item).length > 0" class="relative w-28 h-28 overflow-hidden bg-gray-200 group transition-transform duration-300 cursor-pointer" style="border-radius: .5em;">
                                <!-- Imagen actual -->
                                <img :src="'/storage/' + getCurrentImage(item).devImg_url" :alt="item.devt_title" class="w-full h-full object-cover">
                        ...
                        <td>
                            <span class="inline-flex items-center rounded-md bg-blue-50 px-2 text-blue-700">
                        ...
                        <td>
                            <span class="inline-flex items-center rounded-md bg-purple-50 px-2 text-purple-700">
                                {{ item.business_status?.busiSta_name || 'Sin estado' }}
                            </span>
                        </td>
                        <td>
                            <span class="inline-flex items-center rounded-md bg-green-50 px-2 text-green-700">
                                {{ item.approval_status?.apvSta_name || 'Sin estado' }}
                            </span>
                        </td>
                        <td>
                            <div class="botonera--tabla">
                                <button @click.stop="goToEdit(item.devt_id)" class="btn--tipo1" title="Editar campo" v-if="$page.props.user.permissions.includes('update development') && (!isUserAgenteInmobiliario() || isDevelopmentMine(item))">
                                ...
                                <button @click.stop="router.visit(route('development.captors.index', item.devt_id))" class="btn--tipo1" title="Gestionar Captadores" v-if="$page.props.user.permissions.includes('update development') && (!isUserAgenteInmobiliario() || isDevelopmentMine(item))">
                                    <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z"></path>
                                    </svg>
                                </button>
                                <button @click.stop="deleteDevelopment(item)" class="btn--tipo2" title="Eliminar campo" v-if="$page.props.user.permissions.includes('delete development') && (!isUserAgenteInmobiliario() || isDevelopmentMine(item))">
                                    <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <ConfirmModal
            ...
            message="¿Está seguro que desea eliminar este desarrollo? Esta acción no se puede deshacer."
