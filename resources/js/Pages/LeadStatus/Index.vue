...
import CreateModal from '@/Components/LeadStatus/CreateModal.vue';
import ViewModal from '@/Components/LeadStatus/ViewModal.vue';
...
const showLeadStatusModal = ref(false);
const selectedLeadStatus = ref(null);
const editMode = ref(false);
const showDeleteConfirm = ref(false);
const leadStatusToDelete = ref(null);

const openLeadStatusModal = (leadStatus) => {
    selectedLeadStatus.value = leadStatus;
    editMode.value = false;
    showLeadStatusModal.value = true;
};

const openLeadStatusModalForEdit = (leadStatus) => {
    selectedLeadStatus.value = leadStatus;
    editMode.value = true;
    showLeadStatusModal.value = true;
};
...
const closeLeadStatusModal = () => {
    showLeadStatusModal.value = false;
    selectedLeadStatus.value = null;
    editMode.value = false;
};

const openDeleteConfirm = (leadStatus) => {
    leadStatusToDelete.value = leadStatus;
    showDeleteConfirm.value = true;
};

const confirmDelete = () => {
    if (leadStatusToDelete.value) {
        router.delete(route('leadstatus.destroy', leadStatusToDelete.value.leadSta_id), {
            onSuccess: () => {
                showDeleteConfirm.value = false;
                leadStatusToDelete.value = null;
                
                notificationStore.success('Estado de lead eliminado exitosamente');
            ...
            onError: (error) => {
                ...
                notificationStore.error('Error al eliminar el estado de lead');
                showDeleteConfirm.value = false;
                leadStatusToDelete.value = null;
            }
        });
...
onMounted(() => {
    if (sessionStorage.getItem('showCreateLeadStatusNotification')) {
        setTimeout(() => {
            notificationStore.success('Estado de lead creado exitosamente');
            sessionStorage.removeItem('showCreateLeadStatusNotification');
    ...
    if (sessionStorage.getItem('showUpdateLeadStatusNotification')) {
        setTimeout(() => {
            notificationStore.success('Estado de lead actualizado exitosamente');
            sessionStorage.removeItem('showUpdateLeadStatusNotification');
...
<template>
    <AppLayout>
        <template #header>
            <h1 class="font-semibold text-xl text-gray-800 leading-tight">Estado de Lead</h1>
        </template>

        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-end pr-2" v-if="$page.props.user.permissions.includes('create leads status')">
                        <button @click="showCreateModal = true" class="text-white bg-indigo-500 hover:bg-indigo-700 py-2 px-4 rounded">
                            Crear Estado
                        </button>
                    </div>
                </div>
                                    ...
                                    <table class="w-full rounded-xl">
                                        <thead>
                                            <tr class="bg-gray-50">
                                                ...
                                                <th scope="col" class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize">Código</th>
                                        ...
                                        <tbody class="divide-y divide-gray-300">
                                            <tr v-for="item in lead_statuses.data" :key="item.leadSta_id" class="bg-white transition-all duration-500 hover:bg-gray-50">
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ item.leadSta_id }}</td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ item.leadSta_code }}</td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ item.leadSta_name }}</td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ item.leadSta_description }}</td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    <div :class="['py-1.5 px-2.5 rounded-full flex justify-center w-24 items-center gap-2', item.leadSta_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600']">
                                                        <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <circle cx="3" cy="3" r="3" :fill="item.leadSta_active ? '#059669' : '#dc2626'" />
                                                        </svg>
                                                        <span class="font-semibold text-xs">{{ item.leadSta_active ? 'Activo' : 'Inactivo' }}</span>
                                                    </div>
                                                </td>
                                                <td class="flex p-5 items-center gap-0.5">
                                                    <button
                                                        @click="openLeadStatusModal(item)"
                                                    ...
                                                    <button
                                                        ...
                                                        v-if="$page.props.user.permissions.includes('delete lead status')"
                ...
                <div class="flex items-center justify-between border-t border-gray-200 px-4 py-3 sm:px-6">
                    ...
                    <div class="flex flex-1 justify-between sm:hidden">
                        <Link v-if="lead_statuses.prev_page_url"
                            :href="lead_statuses.prev_page_url"
                            class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                            Anterior
                        </Link>
                        <Link v-if="lead_statuses.next_page_url"
                            :href="lead_statuses.next_page_url"
                    ...
                    <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm text-gray-700">
                                Mostrando
                                <span class="font-medium">{{ lead_statuses.from }}</span>
                                a
                                <span class="font-medium">{{ lead_statuses.to }}</span>
                                de
                                <span class="font-medium">{{ lead_statuses.total }}</span>
                                resultados
                            </p>
                        </div>
                        ...
                        <div>
                            <nav aria-label="Pagination"
                                ...
                                <!-- Page Numbers -->
                                <Link v-for="page in lead_statuses.last_page" :key="page"
                                    :href="`?page=${page}`" :class="[
                                        'relative inline-flex items-center px-4 py-2 text-sm font-semibold focus:z-20 focus:outline-offset-0',
                                        page === lead_statuses.current_page
        ...
        <ConfirmModal
            ...
            title="Eliminar Estado de Lead"
            message="¿Estás seguro de que deseas eliminar este estado de lead? Esta acción es irreversible."
