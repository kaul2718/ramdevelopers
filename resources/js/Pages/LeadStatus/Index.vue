<!-- filepath: resources/js/Pages/LeadStatus/Index.vue -->
<script>
export default {
...
const deleteLeadStatus = (id) => {
    if (confirm("¿Seguro que deseas eliminar este estado?")) {
        router.delete(route("leadstatus.destroy", id));
    }
};
</script>
                                ...
                                <div class="overflow-x-auto w-full border rounded-lg border-gray-300">
                                    <table class="w-full rounded-xl">
                                        ...
                                        <tbody class="divide-y divide-gray-300">
                                            <tr 
                                                v-for="item in lead_statuses.data" 
                                                :key="item.leadSta_id"
                                                ...
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    {{ item.leadSta_description }}
                                                </td>
                                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                    <div :class="[
                                                        'py-1.5 px-2.5 rounded-full flex justify-center w-24 items-center gap-2',
                                                        item.leadSta_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'
                                                    ]">
                                                        <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <circle cx="3" cy="3" r="3" :fill="item.leadSta_active ? '#059669' : '#dc2626'" />
                                                        </svg>
                                                        <span class="font-semibold text-xs">
                                                            {{ item.leadSta_active ? 'Activo' : 'Inactivo' }}
                                                        </span>
                                                    </div>
                                                </td>
                                                <td class="flex p-5 items-center gap-0.5">
                                                    <Link :href="route('leadstatus.edit', item.leadSta_id)"
                                                    ...
                                                    <Link @click="deleteLeadStatus(item.leadSta_id)"
