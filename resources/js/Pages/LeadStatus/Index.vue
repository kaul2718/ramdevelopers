    ...
    <AppLayout title="Estados lead">
        ...
        <div class="caja--tabla">
            <table>
                ...
                <tbody>
                    <tr v-for="item in lead_statuses.data" :key="item.leadSta_id" @click="openLeadStatusModal(item)">
                        <td>{{ item.leadSta_id }}</td>
                        <td>{{ item.leadSta_code }}</td>
                        <td>{{ item.leadSta_name }}</td>
                        <td>{{ item.leadSta_description }}</td>
                        <td>
                            <div :class="['px-2 rounded-full flex justify-center w-24 items-center gap-2', item.leadSta_active ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600']">
                                <svg width="6" height="6" viewBox="0 0 6 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <circle cx="3" cy="3" r="3" :fill="item.leadSta_active ? '#059669' : '#dc2626'" />
                                </svg>
                                <span>{{ item.leadSta_active ? 'Activo' : 'Inactivo' }}</span>
                            </div>
                        </td>
                        <td class="botonera--tabla">
                            <button @click.stop="openLeadStatusModalForEdit(item)" class="btn--tipo1" title="Editar campo" v-if="$page.props.user.permissions.includes('update lead status')">
                            ...
                            <button @click.stop="openDeleteConfirm(item)"class="btn--tipo2" title="Eliminar campo" v-if="$page.props.user.permissions.includes('delete lead status')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
