...
<template>
    <AppLayout title="Estados lead">
        <HeaderBody titulo-body="Estados de Leads" permisos="create leads status" nombre-btn="Crear nuevo" @create="showCreateModal = true"/>

        <div class="caja--tabla">
            <table>
                <thead>
                    <tr>
                        <th>Id</th><th>Código</th><th>Nombre</th><th>Descripción</th><th>Status</th><th>Acciones</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="item in lead_statuses.data" :key="item.leadSta_id">
                        <td>{{ item.leadSta_id }}</td>
                        <td>{{ item.leadSta_code }}</td>
                        <td>{{ item.leadSta_name }}</td>
                        <td>{{ item.leadSta_description }}</td>
                        <td>
                        ...
                        <td class="botonera--tabla">
                            <button @click="openLeadStatusModal(item)" class="btn--tipo1" title="Ver información">
                            ...
                            <button @click="openLeadStatusModalForEdit(item)" class="btn--tipo1" title="Editar campo" v-if="$page.props.user.permissions.includes('update lead status')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <button @click="openDeleteConfirm(item)"class="btn--tipo2" title="Eliminar campo" v-if="$page.props.user.permissions.includes('delete lead status')">
                                <svg class="size-5" viewBox="0 0 20 20" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Paginación -->
        <Pagination :objPage="lead_statuses" />
