...
<template>
    <div v-if="show" class="modal">
        ...
        <div class="caja--modal">
            ...
            <!-- Acciones del Modal -->
            <div class="flex gap-2 mb-4 pb-4 border-b border-gray-300">
                <PrimaryButton @click="openCreateNoteModal">
                    <template #texto--boton>Agregar Nota</template>
            ...
            <div class="contenido--modal">
                <div v-if="lead?.notes && lead.notes.length > 0" class="space-y-4">
                    <div v-for="note in [...lead.notes].reverse()" :key="note.leadNot_id"
                        class="p-3 border border-gray-300 rounded-md hover:border-indigo-400 transition">
                        <div class="flex justify-between items-start mb-2">
                            <div class="flex-1">
                                <h4 class="font-semibold">{{ note.leadNot_title }}</h4>
                            </div>
                            <div class="flex gap-2 ml-4">
                                <button @click="openEditNoteModal(note)"
                                    class="btn--tipo1" title="Editar nota">
                        ...
                        <p class="text-sm mb-2">{{ note.leadNot_description }}</p>
                ...
                <div v-else class="text-center py-8">
                    <p class="text-gray-600">No hay notas asociadas a este lead</p>
                </div>
            </div>
        </div>
    </div>
